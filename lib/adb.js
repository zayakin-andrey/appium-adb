...
ADB.prototype.broadcastIntent = function (intent, pkg, receiver, keys, cb) {
  var cmd = 'am broadcast -a ' + intent + ' -n ' + pkg + '/' + receiver;
  for (var key in keys) {
...
ADB.prototype.getDate = function (cb) {
  var cmd = 'date';
  this.shell(cmd, function (err, stdout) {
    var match = /\s(\w{3})\s*?(\d+)\s(\d+):(\d+):(\d+)\s(\w+)\s(\d{4})/gi.exec(stdout);
    if (match) {
      var month = new Date(match[1] + " 1, 2000").getMonth(),
          day = match[2],
          hour = match[3],
          minutes = match[4],
          seconds = match[5],
          offset = timeOffset[match[6]] * 60 * 1000,
          year = match[7];
      cb(null, new Date(year, month, day, hour, minutes, seconds).getTime() + offset);
    } else {
      cb("Can't parse information about current date: " + stdout);
    }
  });
};

...
var spawn = require('child_process').spawn
    ...
    , timeOffset = require('timezone-abbr-offsets');
...
ADB.prototype.rimraf = function (path, cb) {
  this.hasSudo(function (err, sudo) {
    var cmd = sudo ? 'su -c rm -rf ' + path : 'rm -rf ' + path;
    this.shell(cmd, cb);
  }.bind(this));
};
...
ADB.prototype.push = function (localPath, remotePath, cb) {
  ...
  this.exec('push ' + localPath + ' ' + remotePath.trim().replace(/\s/g, '\\ '), cb);
};
...
ADB.prototype.pull = function (remotePath, localPath, cb) {
  ...
  localPath = JSON.stringify(localPath);
  this.exec('pull ' + remotePath.trim().replace(/\s/g, '\\ ') + ' ' + localPath, cb);
};
...
ADB.prototype.launchAVD = function (avdName, avdArgs, language, locale, avdLaunchTimeout,
                                    avdReadyTimeout, cb, retry, sdcard) {
  ...
  this.checkSdkBinaryPresent("emulator", function (err, emulatorBinaryPath) {
    ...
    if (typeof avdArgs === "string") {
      launchArgs = launchArgs.concat(avdArgs.split(" "));
    }
    logger.debug("Spawn emulator: " + emulatorBinaryPath + " " + launchArgs.join(' '));
    ...
    proc.stdout.on('data', function (data) {
      if (data.toString().indexOf('ERROR') > -1) {
        ...
        logger.debug("EMULATOR: " + data.toString());
      }
    });
          ...
          if (err) {
            if (retry < 1) {
              ...
              return this.launchAVD(avdName, avdArgs, language, locale, avdLaunchTimeout,
                  avdReadyTimeout, cb, retry + 1, sdcard);
            } else {
              return cb(err);
            }
          }
          this.waitForEmulatorReady(avdReadyTimeout, function (err) {
            if (err) {
              if (retry < 1) {
                logger.warn("Emulator never became ready. Going to retry once");
                proc.kill();
                return this.launchAVD(avdName, avdArgs, language, locale, avdLaunchTimeout,
                    avdReadyTimeout, cb, retry + 1, sdcard);
              } else {
              ...
              if (sdcard) {
                this.isSdcardMounted(function (mounted) {
                  if (mounted) {
                    cb();
                  } else {
                    if (retry < 1) {
                      logger.warn("Sdcard didnt mount. Going to retry once");
                      proc.kill();
                      this.killAVD(function () {
                        return this.launchAVD(avdName, avdArgs, language, locale, avdLaunchTimeout,
                            avdReadyTimeout, cb, retry + 1, sdcard);
                      }.bind(this));
                    } else {
                      return cb("Sdcard didnt mount");
                    }
                  }
                }.bind(this));
              } else {
...
var _buildStartCmd = function (startAppOptions, apiLevel) {
  ...
  if (startAppOptions.data) {
    cmd += " -d " + startAppOptions.data;
  }
...
ADB.prototype.startApp = function (startAppOptions, cb) {
  ...
  this.getApiLevel(function (err, apiLevel) {
    ...
    this.shell(cmd, function (err, stdout) {
      ...
      } else if (startAppOptions.waitPkg) {
        if (startAppOptions.hasOwnProperty("waitDuration")) {
          this.waitForPackage(startAppOptions.waitPkg, startAppOptions.waitDuration, cb);
        } else {
          this.waitForPackage(startAppOptions.waitPkg, cb);
        }
      } else {
        cb();
      }
...
ADB.prototype.startAction = function (action, category, data, cb) {
  if (!this.isValidClass(action)) return cb(new Error("Invalid action " + action));

  var cmd = _buildStartCmd({
    action: action,
    category: category,
    data: data
  });
  logger.debug("Starting action: " + cmd);
  this.shell(cmd, cb);
};
...
ADB.prototype.waitForPackageOrNot = function (pkg, not, waitMs, cb) {
  ...
  logger.debug("Waiting for pkg \"" + pkg + "\" to " + (not ? "not " : "") + "be focused");
  var intMs = 750
      , endAt = Date.now() + waitMs;

  var checkForPackage = function (foundPackage) {
    return foundPackage === pkg;
  };

  var wait = function () {
    this.getFocusedPackageAndActivity(function (err, foundPackage) {
      if (err) return cb(err);
      var foundAct = checkForPackage(foundPackage);
      if ((!not && foundAct) || (not && !foundAct)) {
        cb();
      } else if (Date.now() < endAt) {
        setTimeout(wait, intMs);
        ...
        var verb = not ? "stopped" : "started";
        var msg = pkg + " never " + verb + ". Current: " + foundPackage;
        logger.error(msg);
        cb(new Error(msg));
      }
    }.bind(this));
...
ADB.prototype.waitForPackage = function (pkg, waitMs, cb) {
  this.waitForPackageOrNot(pkg, false, waitMs, cb);
};

ADB.prototype.waitForNotPackage = function (pkg, waitMs, cb) {
  this.waitForPackageOrNot(pkg, true, waitMs, cb);
};
...
ADB.prototype.uninstallApks = function (pkgs, cb) {
  if (Array.isArray(pkgs)) {
    if (pkgs.length > 0) {
      var i = 0, uninstalled = 0;
      var result = function (err) {
        if (err) {
          cb(err);
        } else {
          uninstalled++;
          if (uninstalled == pkgs.length) {
            cb();
          }
        }
      }.bind(this);
      while (pkgs[i]) {
        this.uninstallApk(pkgs[i++], result);
      }
    } else {
      cb();
    }
  } else {
    this.uninstallApk(pkgs, cb);
  }
};
...
ADB.prototype.getMatchedPackages = function (pattern, cb) {
  logger.debug("Getting packages match string " + pattern);
  this.getApiLevel(function (err, apiLevel) {
    if (err) return cb(err);
    var thirdparty = apiLevel >= 15 ? "-3 " : "";
    var listPkgCmd = "pm list packages " + thirdparty + " | grep " + pattern;
    this.shell(listPkgCmd, function (err, stdout) {
      if (err) return cb(err);
      var apkInstalledRgx = new RegExp('^package:(.*?)$', 'gm'),
          match,
          installed = [];
      while ((match = apkInstalledRgx.exec(stdout)) !== null) {
...
ADB.prototype.goToHome = function (cb) {
  ...
  this.keyevent(3, false, cb);
};

ADB.prototype.keyevent = function (keycode, longpress, cb) {
  var code = parseInt(keycode, 10);
  // keycode must be an int.
  var cmd = ['input', 'keyevent'];
  if (longpress) {
    cmd.push('--longpress');
  }
  cmd.push(code);
  this.shell(cmd.join(' '), cb);
};
...
ADB.prototype.getVersionFromManifest = function (localApk, cb) {
  this.checkAaptPresent(function (err) {
    if (err) return cb(err);
    logger.debug("Get package version from manifest.");
    prettyExec(this.binaries.aapt,
        ['dump', 'badging', localApk],
        { maxBuffer: 524288 }, function (err, stdout, stderr) {
          if (err || stderr) {
            logger.warn(stderr);
            return cb(new Error("getVersionFromManifest. " + err));
          }
          var version = /versionCode.*?'(.*?)'.*?versionName.*?'(.*?)'/gi.exec(stdout);
          if (version) {
            cb(null, {
              code: version[1],
              name: version[2]
...
ADB.prototype.ls = function (dir, cb) {
  var cmd = 'ls ' + dir;
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
...
ADB.prototype.rm = function (path, cb) {
  this.shell('rm ' + path, function (err, stdout) {
    if(new RegExp("rm failed for .*, No such file or directory", "i").test(stdout)) {
      cb(stdout);
    } else {
      cb(null);
    }
...
ADB.prototype.broadcastIntent = function (intent, pkg, receiver, keys, cb) {
  var cmd = 'am broadcast -a ' + intent;
  if (receiver) {
    cmd += ' -n ' + pkg + '/' + receiver;
  }
...
ADB.prototype.tap = function (x, y, cb) {
  var cmd = ['input', 'tap', x, y, '&'];
  this.shell(cmd.join(' '), function (err) {
    if (err) return cb(err);
    cb(null);
  });
};

ADB.prototype.swipe = function (x1, y1, x2, y2, duration, sleep, cb) {
  var cmd = ['input', 'swipe', x1, y1, x2, y2, duration];
  if (sleep) {
    cmd.push('&', 'sleep', sleep);
  }
  this.shell(cmd.join(' '), function (err) {
    if (err) return cb(err);
    cb(null);
...
ADB.prototype.getNavigationBarRegion = function (cb) {
  var cmd = 'dumpsys input | grep -oE "NavigationBar.*touchableRegion.*"';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    var inputMatch = /touchableRegion=\[(\d+),(\d+)]\[(\d+),(\d+)]/gi.exec(stdout);
    if (inputMatch) {
      cb(null, {
        x1: inputMatch[1],
        y1: inputMatch[2],
      ...
      cb("Can't find NavigationBar");
    }
  });
};

ADB.prototype.getDisplayMetrics = function (cb) {
  var cmd = 'dumpsys display | grep -A 20 DisplayDeviceInfo';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    var inputMatch = /\s(\d+)\sx\s(\d+).*?density\s(\d+).*?([\d\.]+).*?([\d\.]+).*?dpi/gi.exec(stdout),
        scaleFactorMatch = /PhysicalDisplayInfo.*?density\s([\d\.]+)/gi.exec(stdout);
    if (inputMatch && scaleFactorMatch) {
      cb(null, {
        width: inputMatch[1],
        height: inputMatch[2],
        density: inputMatch[3],
        wdpi: inputMatch[4],
        hdpi: inputMatch[5],
        scaleFactor: scaleFactorMatch[1]
      });
    } else {
      cb("Can't find information about display");
    }
  });
};

ADB.prototype.hasSudo = function (cb) {
  var cmd = 'su 0 ls';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    cb(null, stdout.indexOf('not found') === -1);
  });
};

ADB.prototype.getDateString = function (cb) {
  var cmd = 'date';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    cb(null, stdout);
...
ADB.prototype.getDate = function (cb) {
  var cmd = 'date +%s';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    cb(null, stdout);
...
ADB.prototype.setDate = function (time, cb) {
  var cmd = 'su 0 date -s ',
      date = new Date(),
      format = function (v) {
        return v > 9 ? v : '0' + v
  ...
  this.shell(cmd, function (err) {
    if (err) return cb(err);
    cb(null);
  });
};

ADB.prototype.inputText = function (text, cb) {
  var i = 0,
      arr = text.split(' '),
      sendText = function () {
        if (typeof arr[i] !== 'undefined') {
          this.shell('input text ' + arr[i++], function (err) {
            if (err) return cb(err);
            if (typeof arr[i] !== 'undefined') {
              this.shell('input keyevent 62', function (err) {
                if (err) return cb(err);
                sendText();
              });
            } else {
...
ADB.prototype.killAVD = function (cb) {
  logger.debug("Trying to shutdown emulator");
  // emu kill just does nothing on a real device
  this.getEmulatorPort(function (err, port) {
    if (err) return cb(err);
    this.exec("emu kill", function (err, stdout) {
      this.freePort(port, function () {
        if (err) return cb(err);
        if (stdout.toLowerCase().indexOf("error") !== -1) {
          logger.error(stdout);
        }
...
ADB.prototype.freePort = function (port, cb) {
  var stdout = '',
      netstat = spawn('netstat', ['-a', '-n', '-p']);
  ...
  netstat.on('close', function () {
    var match = new RegExp(':' + port + '\\s.*?\\s(\\d+)/java', 'gi').exec(stdout);
    if (match) {
      var id = match[1];
      prettyExec('kill', [id], {maxBuffer: 524288}, function (err) {
        if (err) logger.debug("Could not kill process.: " + err.message);
        cb();
      });
    } else {
      logger.debug("Could not find port");
      cb();
    }
  });
};

ADB.prototype.getVersions = function (pkg, cb) {
  var cmd = 'dumpsys package ' + pkg + ' | grep -A 1 versionCode';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    var match,
        regexp = /versionCode=(\d+)[\s\S]*?versionName=([^\n|\r]+)/gim,
        versions = [];
    ...
    if (versions.length) {
      ...
      cb("Could not find pkg version");
    }
  });
};

ADB.prototype.getOwner = function (path, cb) {
  this.hasSudo(function (err, sudo) {
    var cmd = sudo ? 'su -c ls -ld ' + path : 'ls -ld ' + path;
    this.shell(cmd, function (err, stdout) {
      if (err) return cb(err);
      var match = new RegExp('[^\\s]+\\s+([^\\s]+)\\s+([^\\s]+)\\s+\\d*?\\s+\\d{4}\\-.*?', 'gim').exec(stdout);
      if (match) {
        cb(null, {
          user: match[1],
          group: match[2]
        ...
        cb("Could not find path " + path);
      }
    });
  }.bind(this));
};

ADB.prototype.setOwner = function (file, user, group, cb) {
  var cmd = 'su -c chown ' + user + ':' + group + ' ' + file;
  this.shell(cmd, function (err) {
    if (err) cb("Set owner error");
    cb();
  });
};

ADB.prototype.isDir = function (p, cb) {
  var cmd = 'cd ' + p;
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    cb(null, stdout.indexOf('Not a directory') == -1);
  });
};

ADB.prototype.listFolder = function (dir, cb) {
  this.hasSudo(function (err, sudo) {
    var cmd = sudo ? 'su -c find ' + dir : 'find ' + dir;
    this.shell(cmd, function (err, stdout) {
      if (err) return cb(err);
      cb(null, stdout.split("\r\n"));
    });
...
ADB.prototype.cp = function (source, destination, cb) {
  this.hasSudo(function (err, sudo) {
    var cmd = sudo ? 'su -c cp -R ' + source + ' ' + destination  : 'cp -R ' + source + ' ' + destination;
    this.shell(cmd, function (err) {
      if (err) return cb(err);
      cb();
    });
...
ADB.prototype.isSdcardMounted = function (cb) {
  this.shell('df', function (err, stdout) {
    if (err) return cb(err);
    cb(null, stdout.indexOf('sdcard') > -1);
  });
};

...
ADB.prototype.freePort = function (port, cb) {
  prettyExec('netstat', ['-a', '-n', '-p'], { maxBuffer: 524288 }, function (err, stderr, stdout) {
    if (err) {
      logger.debug("Could not execute netstat.: " + err.message);
      cb();
    } else {
      logger.debug("stdout: " + stdout);
      logger.debug("port: " + port);
      var match1 = new RegExp(':' + port + '\\s.*?\\s(\\d+)/java', 'gi').exec(stdout);
      var match2 = new RegExp(':' + port + '\\s.*?\\s\(\\d+\)/java', 'gi').exec(stdout);
      if (match1 || match2) {
        var id = match[1];
